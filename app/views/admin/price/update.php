                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-4 col-md-6 d-flex flex-column align-items-center justify-content-center">
                            ...
                            <div class="card mb-3">
                                <div class="card-body">
                                    <div class="pt-4 pb-2">
                                        <h5 class="card-title text-center pb-0 fs-4">Update Ticket Price</h5>
                                    </div>
                                    ...
                                    <?php endif; ?>
                                    <form action="<?php echo __WEB_ROOT__ ?>/price/update/<?php echo $price['id'] ?>"
                                        class="row g-3 needs-validation" novalidate method="POST">
                                        <div class="col-12">
                                            <label class="form-label">Schedule</label>
                                            <select name="schedule_id" class="form-control form-control-line">
                                                <option value="">Select schedule</option>
                                                <?php if(!empty($schedules)) :?>
                                                <?php foreach($schedules as $key=>$value) :?>
                                                <option value="<?php echo $value['id']?>"
                                                    <?php echo $value['id'] == $price['schedule_id'] ? 'selected' : '' ?>>
                                                    <?php echo $value['movie_name'] . " - " . $value['cinema_name'] . " - " 
                                                    . $value['room_name'] . " - " . date("d-m-Y", strtotime($value["schedule_date"]))?>
                                                </option>
                                                <?php endforeach;?>
                                                <?php endif; ?>
                                            </select>
                                        ...
                                        <div class="col-12">
                                            <label class="form-label">Seate Type</label>
                                            <select name="seat_type_id" class="form-control form-control-line">
                                                <option value="">Select Seate</option>
                                                <?php if(!empty($seatType)) :?>
                                                <?php foreach($seatType as $key=>$value) :?>
                                                <option value="<?php echo $value['id']?>"
                                                    <?php echo $value['id'] == $price['seat_type_id'] ? 'selected' : '' ?>>
                                                    <?php echo $value['seat_type'] ?>
                                                </option>
                                                <?php endforeach;?>
                                                <?php endif; ?>
                                            </select>
                                        ...
                                        <div class="col-12">
                                            ...
                                            <input type="text" name="price" class="form-control"
                                                value="<?php echo isset($_POST['price']) ? $_POST['price'] : $price['price']; ?>">
                                        </div>
                                        <div class="col-12">
                                            <button class="btn btn-primary w-100" type="submit">Update Ticket
                                                Price</button>
                                        </div>
                                    </form>
                                </div>
